...
@section('content')
    ...
    <div class="container">
        ...
        <div class="card">
            <form action="{{ url('admin/attendance/update-entry') }}" method="post" class="needs-validation" autocomplete="off" novalidate accept-charset="utf-8">
                @csrf
                <input type="hidden" name="entry_to_update" value="{{ $entry->id }}">
                <div class="card-header"></div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="name" class="form-label">{{ __("Employee") }}</label>
                        <select name="name" class="form-select select-search" required>
                            <option value="{{ $entry->employee }}" selected disabled data-reference="{{ $entry->reference }}">{{ $entry->employee }}</option>
                        </select>
                    </div>

                    <div class="mb-3">
                        <label for="date" class="form-label">{{ __("Date") }}</label>
                        <input type="text" name="date" value="{{ $entry->date }}" class="airdatepicker form-control"  required>
                    </div>

                    <div class="row g-2">
                        <div class="mb-3 col-md-6">
                            <label for="clockin" class="form-label">{{ __("Clock IN") }} <small class="text-muted">({{ __("required") }})</small></label>
                            <input type="text" name="clockin" value="{{ date('H:i:s', strtotime($entry->timein)) }}" class="timepicker form-control"  required>
                        </div>
                        <div class="mb-3 col-md-6">
                            <label for="clockout" class="form-label">{{ __("Clock OUT") }}</label>
                            <input type="text" name="clockout" value="{{ date('H:i:s', strtotime($entry->timeout)) }}" class="timepicker form-control" >
                        </div>
                    </div>
