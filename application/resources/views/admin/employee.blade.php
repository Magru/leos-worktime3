                ...
                <div class="row g-1">
                    ...
                    <div class="col-md-6 mb-1 text-start">
                        <a href="{{ url('/admin/employee/add') }}" class="btn btn-outline-primary btn-sm">
                            <span class="button-with-icon">{{ __("Add") }}</span><i class="fas fa-plus"></i>
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    @isset($employees)
                        @foreach ($employees as $employee)
                            <tr class="@if($employee->employmentstatus != 'Active') red-cell @else  @endif">
                                ...
                                <td><span class="text-uppercase">@if($employee->employmentstatus == 'Active')
                                            פעיל
                                        @else
                                            ארכיון
                                        @endif</span></td>
                                <td class="text-end text-center">
                                    ...
                                    <a href="{{ route('emp-attendance', ['emp_id' => $employee->id]) }}"
                                       class="btn btn-outline-secondary btn-sm btn-rounded"><i class="fa-solid fa-clock"></i></a>

                                    <a href="{{ url('/admin/employee/edit') }}/{{ $employee->reference }}"
                                       class="btn btn-outline-secondary btn-sm btn-rounded"><i
                                                class="fas fa-pen"></i></a>

                                    <a href="{{ url('/admin/employee/archive') }}/{{ $employee->reference }}"
                                       class="btn btn-outline-secondary btn-sm btn-rounded"><i
                                                class="fas fa-archive"></i></a>

                                    <a href="#"
                                       data-url="{{ url('/admin/employee/delete') }}/{{ $employee->reference }}"
