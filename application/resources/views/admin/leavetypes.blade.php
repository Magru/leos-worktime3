            ...
            <div class="card mb-2">
                <div class="card-body">
                    <form action="{{ url('admin/leavetype/add') }}" method="post" class="needs-validation" autocomplete="off" novalidate accept-charset="utf-8">
                        @csrf
                        <div class="mb-3">
                            <label for="leavetype" class="form-label">{{ __("Leave Name") }} <small class="text-muted">לדוגמא: חופשת מחלה, חופשת לידה</small></label>
                            <input type="text" name="leavetype" value="" class="form-control text-uppercase" required>
                        </div>

                        <div class="mb-3">
                            <label for="credits" class="form-label">{{ __("Credits") }} <small class="text-muted">לדוגמא: 15 (ימים)</small></label>
